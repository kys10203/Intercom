...
// 참고: IID_IMWOD_LOAD2에 대한 지원을 추가하여
...
void CMWOD_LOAD2::CheckPresentARC2Setting(void)
{
    ...
    if(1 == ((CSET_ARC2 *)GetParent())->m_combo_arc2_mode.GetCurSel()) // 프리셋 모드
    {
        ((CSET_ARC2 *)GetParent())->m_edit_arc2_channel.GetWindowTextW(str);
        ((CSET_ARC2 *)GetParent())->un_PRESET_SELECT.st_PRESET_SELECT.ChannelNumber = (U16BIT)(_ttoi(str)); // GUI (Preset Mode 시, Channel Number 입력(01~99))
        ((CSET_ARC2 *)GetParent())->un_PRESET_SELECT.st_PRESET_SELECT.MemoryManagement = 0x1; // Preset Mode에서 명령 전송 시 : 입력한 Channel Number 및 Memory Managemet=0001(Tune Channel) 전송(Subaddr 01)
    }
    else if(0 == ((CSET_ARC2 *)GetParent())->m_combo_arc2_mode.GetCurSel()) // 매뉴얼 모드
    {
        GetDlgItemText(IDC_EDIT_ARC2_FREQ, str);
        ((CSET_ARC2 *)GetParent())->un_PRESET_SELECT.st_PRESET_SELECT.MemoryManagement = 0x4; // Manual Mode에서 명령 전송 시 : 입력한 Frequency 및 Memory Managemet=0100(Tune Direct) 전송(Subaddr 01)
    ...
    {
    case 0:
        ((CSET_ARC2 *)GetParent())->un_MODE_AND_GUARD.st_MODE_AND_GUARD.GuardReceiver = 0x0;   // T/R Mode : Gurad Receiver(BC-RT, SA01, W02, b04)=0(Disable) 전송
        break;
    case 1:
        ((CSET_ARC2 *)GetParent())->un_MODE_AND_GUARD.st_MODE_AND_GUARD.GuardReceiver = 0x1;   // T/R+G Mode : Guard Receiver(BC-RT, SA01, W02, b04)=1(Enable) 전송
        break;
    case 2:
        ((CSET_ARC2 *)GetParent())->un_PRESET_SELECT.st_PRESET_SELECT.MemoryManagement = 0x7; // G Mode : Memory Management(BC-RT, SA01, W01, b08-11)=0111(Guard) 전송
        break;
    }
